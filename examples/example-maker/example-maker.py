...
QUESTIONS = (
    ...
    Question(
        ...
        "Absolute paths are clearest. Relative paths start from the directory where you launched the maker.",
        multiline=True,
    ),
    Question(
        "problem_globs",
        ...
        "The files may live anywhere; they are packaged only when the generated submission runs.",
        multiline=True,
    ),
    Question(
        "cpu_limit",
...
def workflow_makefile(workflow: Workflow) -> str:
    return "\n".join(
        [
            "# Generated by example-maker.py. Paths refer to existing files; nothing was copied here.",
...
This workflow references preexisting solver descriptions and benchmark files.
They were not copied into this directory. `slurmy.py` reads the descriptions
when `make` runs, and the generated `submit.sh` packages their solver roots and
the matched problems when the job is submitted.
...
class ExampleMakerApp(App[None]):
    ...
    def validate_answer(self, question: Question, value: str) -> tuple[Any, str]:
        ...
        if question.key == "solver_files":
            _, matches = expand_regular_files(
                value, base=self.launch_directory, description="solver"
            )
        ...
        if question.key == "problem_globs":
            patterns, matches = expand_regular_files(
                value, base=self.launch_directory, description="problem"
            )
    ...
    def save_current_answer(self) -> bool:
        ...
        if question.key in {"solver_files", "problem_globs"}:
    ...
    def workflow(self) -> Workflow:
        missing = [question.key for question in QUESTIONS if question.key not in self.answers]
        if missing:
        ...
        return Workflow(
            ...
            solver_files=self.answers["solver_files"],
    ...
    def show_review(self) -> None:
        ...
        self.query_one("#explanation", Static).update(
            "No solver or problem files will be copied. The generated workflow keeps absolute references to the validated locations below."
        )
    ...
    def create_workflow(self) -> None:
        ...
        self.query_one("#review", Static).update(
            ...
            "No solver or problem files were copied. Review workflow.mk, then run make."
        )
