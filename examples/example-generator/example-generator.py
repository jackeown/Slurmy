...
try:
    ...
    if exc.name == "textual" or (exc.name and exc.name.startswith("rich")):
        ...
        print(
            "The Slurmy example generator needs Textual. Install it with:\n"
...
INITIAL_QUESTIONS = (
    ...
    Question(
        "remote_build",
        "Should Slurmy build the solver on the cluster?",
        "Choose yes to create a Slurm build job, or no to use a solver installation that already exists locally.",
        "Enter exactly: yes or no",
    ),
)

SOLVER_MODE_QUESTIONS = (
    Question(
        "solver_mode",
...
REMOTE_BUILD_QUESTIONS = (
    Question(
        "build_context",
        "Should a local source directory be sent to the remote build?",
        "The entire directory is archived locally, extracted into $SLURMY_BUILD_WORK on the compute node, and available to the build steps.",
        "Enter an existing directory path, or exactly: none",
    ),
    Question(
        "build_commands",
        "What steps build and install the solver executable?",
        "These Bash lines run on a Slurm compute node. Put the finished executable below $SLURMY_BUILD_OUTPUT.",
        "You may clone sources or build the supplied context. $SLURMY_BUILD_WORK, $SLURMY_BUILD_OUTPUT, and $SLURM_CPUS_PER_TASK are available.",
        multiline=True,
    ),
    Question(
        "solver_artifact",
        "What executable path will the build produce?",
        "This is the path below $SLURMY_BUILD_OUTPUT that Slurmy must download and validate.",
        "For example: vampire or bin/my-solver",
    ),
    Question(
        "solver_commands",
        "How should the remotely built solver be invoked?",
        "Enter one invocation per line. Commands run from the downloaded bin/ directory packaged as the solver root.",
        "For example: ./vampire --time_limit {{cpu-limit}} {{problem}}",
        multiline=True,
    ),
    Question(
        "build_cpus",
        "How many CPUs should the remote build job request?",
        "The build receives this value through $SLURM_CPUS_PER_TASK.",
        "Enter a positive whole number.",
    ),
    Question(
        "build_memory",
        "How much memory should the remote build job request?",
        "This limit applies to compilation, separately from later solver calls.",
        "For example: 8GiB",
    ),
    Question(
        "build_time",
        "How much wall-clock time should the remote build job request?",
        "Use a Slurm time value.",
        "For example: 00:30:00",
    ),
)
...
def build_artifact(value: str) -> str:
    ...
    if (
        ...
        raise WorkflowError(
            "Enter a relative executable path using letters, numbers, dots, "
            "underscores, plus signs, hyphens, and forward slashes."
        )
    return path.as_posix()


def validate_solver_commands(value: str) -> tuple[str, ...]:
...
def workflow_destination(name: str) -> Path:
    return Path(__file__).resolve().parents[1] / "GENERATED" / f"example-{name}"


def workflow_makefile(workflow: Workflow) -> str:
    values = [
        "# Generated by example-generator.py. Input lists contain absolute paths.",
        "REPO_ROOT := ../../..",
        f"SLURMY_HOST := {workflow.host}",
        f"SLURMY_BUILD_PARTITION := {workflow.partition}",
        f"EXAMPLE_NAME := {workflow.name}",
    ]
    if workflow.remote_build:
        values.extend(
            [
                "SOLVER_PATHS_FILE :=",
                ...
                (
                    ...
                    if workflow.build_context
                    else "BUILD_CONTEXT_FILE :="
                ),
                f"SOLVER_ARTIFACT := {workflow.solver_artifact}",
...
def generated_build_script(workflow: Workflow) -> str:
    if not workflow.build_commands:
        raise WorkflowError("The remote build steps are missing.")
    return """#!/usr/bin/env bash
# Generated remote build recipe. This runs in a Slurm job on a compute node.
set -euo pipefail
...
""" + workflow.build_commands.rstrip() + "\n"


def generated_readme(workflow: Workflow) -> str:
    if workflow.remote_build:
        solver_note = """Running `make` submits `build.sh` as a Slurm build job. Its declared
executable is downloaded into `bin/`, then packaged with the experiment. If a
build context was selected, `build-context.txt` records its existing absolute
location and it is uploaded only when the build starts."""
        rebuild_note = """
If that external build context changes later, run `make distclean` before
`make` to rebuild it on the cluster.
"""
    elif workflow.solver_mode == "create":
...
If referenced files move, update the generated path-list files before running
`make` again.
"""
...
def generate_workflow(workflow: Workflow) -> Path:
    ...
    if workflow.remote_build and any(
        value is None
        for value in (
            workflow.build_commands,
            workflow.solver_artifact,
            workflow.build_cpus,
            workflow.build_memory,
            workflow.build_time,
        )
    ):
        raise WorkflowError("The remote build configuration is incomplete.")
    ...
    try:
        ...
        if workflow.remote_build:
            if not workflow.solver_artifact or not workflow.solver_commands:
                raise WorkflowError("The remote build configuration is incomplete.")
            (staging / "build.sh").write_text(
                generated_build_script(workflow), encoding="utf-8"
            )
            (staging / "build.sh").chmod(0o755)
            if workflow.build_context:
                (staging / "build-context.txt").write_text(
                    workflow.build_context + "\n", encoding="utf-8"
                )
            solver_root = str(destination / "bin")
            (staging / "solver.solver").write_text(
                "\n".join((solver_root, *workflow.solver_commands)) + "\n",
                encoding="utf-8",
            )
        elif workflow.solver_mode == "create":
...
class ExampleGeneratorApp(App[None]):
    TITLE = "Slurmy Example Generator"
    ...
    def __init__(self) -> None:
        ...
        self.generator_directory = Path(__file__).resolve().parent

    def questions(self) -> tuple[Question, ...]:
        remote_build = self.answers.get("remote_build")
        if remote_build is True:
            return INITIAL_QUESTIONS + REMOTE_BUILD_QUESTIONS + FINAL_QUESTIONS
        if remote_build is not False:
            return INITIAL_QUESTIONS + FINAL_QUESTIONS
    ...
    def validate_answer(self, question: Question, value: str) -> tuple[Any, str]:
        ...
        if question.key == "name":
            ...
            destination = workflow_destination(name)
            if destination.exists():
        ...
        if question.key == "solver_files":
            _, matches = expand_regular_files(
                value, base=self.generator_directory, description="solver"
            )
        ...
        if question.key == "solver_root":
            root = existing_directory(
                value, base=self.generator_directory, description="solver root"
            )
            return root, f"Validated solver root: {root}"
        if question.key == "build_context":
            if value.lower() == "none":
                return None, "No local build context will be uploaded."
            context = existing_directory(
                value, base=self.generator_directory, description="build context"
            )
            return context, f"Validated build context: {context}"
        if question.key == "build_commands":
            if "\x00" in value or not lines(value):
                raise WorkflowError("Enter at least one remote build command.")
            return value.strip(), "Accepted."
        if question.key == "solver_artifact":
            artifact = build_artifact(value)
            return artifact, f"Will require: $SLURMY_BUILD_OUTPUT/{artifact}"
        if question.key == "solver_commands":
            commands = validate_solver_commands(value)
        ...
        if question.key == "problem_globs":
            patterns, matches = expand_regular_files(
                value, base=self.generator_directory, description="problem"
            )
        ...
        if question.key in {"memory_limit", "memory_request", "build_memory"}:
            if not MEMORY_RE.fullmatch(value):
                raise WorkflowError("Use a positive value such as 2GiB or 2300MiB.")
            return value, "Accepted."
        if question.key == "build_time":
            if not re.fullmatch(r"(?:[0-9]+-)?[0-9]{1,2}(?::[0-9]{2}){0,2}", value):
                raise WorkflowError(
                    "Use a Slurm time containing digits, colons, or a day separator."
                )
            return value, "Accepted."
    ...
    def save_current_answer(self) -> bool:
        ...
        try:
            ...
            return False
        if question.key == "remote_build" and answer != self.answers.get("remote_build"):
            for key in (
                "solver_mode",
                "solver_files",
                "solver_root",
        ...
        elif question.key == "solver_mode" and answer != self.answers.get("solver_mode"):
    ...
    def show_question(self) -> None:
        ...
        continue_button.label = (
            ...
            if question.key
            in {"solver_files", "solver_root", "build_context", "problem_globs"}
            else "Continue"
        )
    ...
    def workflow(self) -> Workflow:
        ...
        return Workflow(
            ...
            remote_build=self.answers["remote_build"],
            solver_mode=self.answers.get("solver_mode", "remote"),
            ...
            build_context=self.answers.get("build_context"),
            build_commands=self.answers.get("build_commands"),
            solver_artifact=self.answers.get("solver_artifact"),
            build_cpus=self.answers.get("build_cpus"),
            build_memory=self.answers.get("build_memory"),
            build_time=self.answers.get("build_time"),
    ...
    def show_review(self) -> None:
        ...
        if workflow.remote_build:
            solver_lines = [
                "Remote build: yes",
                f"Build context: {escape(workflow.build_context or 'none')}",
                f"Executable: bin/{escape(workflow.solver_artifact or '')}",
                (
                    f"Build allocation: {workflow.build_cpus} CPU(s) · "
                    f"{escape(workflow.build_memory or '')} · "
                    f"{escape(workflow.build_time or '')}"
                ),
                "Build steps:",
                *(f"  {escape(line)}" for line in (workflow.build_commands or "").splitlines()),
                "Solver invocations:",
                *(f"  {escape(command)}" for command in workflow.solver_commands),
            ]
        elif workflow.solver_mode == "existing":
            solver_lines = [
    ...
    def create_workflow(self) -> None:
        ...
        self.query_one("#review", Static).update(
            ...
            "Review workflow.mk and the generated files, then run make."
        )
